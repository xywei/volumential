...
`adaptive_split_composition_3d.py` is the 3D companion of that driver on the graded 2:1 trees of `adaptive_timing_3d.py`: a canonical `1/r` base table plus the odd radial power channels `r^(2j-1)`, `j = 1 .. p-1` (so `p - 1` channels and `p` shared tables at retained order `p`, asserted at run time), against per-level per-parameter direct fixed-parameter tables. Its CSV is the 2D driver's schema with a single `dim` column inserted after `kernel`, so one reader consumes both files, and the committed Yukawa online-split column prefix never moves. It carries the same three extensions behind flags: `--kernels Yukawa Helmholtz` (the shared `--parameters` list is read as wave numbers `k`; case ids `helmholtz3d-...-k<param>-...`); `--quadrature-policy {auto,default,high-accuracy}`, whose `auto` default keeps the historical mapping (default policy in smoke, the 3D field demonstrations' high-accuracy policy in full). Unlike 2D, where the high-accuracy policy exists to resolve the Yukawa `K_0` logarithm and Helmholtz stays on the default rule, both 3D kernels carry the canonical `1/r` singularity, so one policy serves both and the two kernels' rows are comparable at equal quadrature. `--include-windowed` appends one `table_strategy=windowed_assembled` row per kernel and parameter through the same assemble/register/pure-cache-load path as the 2D driver, at the assembler's tested 3D channel orders 20/61; the 3D windowed channel family is parameter- *and* kernel-independent, so the first `(kernel, parameter)` pays the cold build and the rest hit cache. The split-order convergence gate is scoped, as in 2D, to full-mode high-accuracy Yukawa online-split rows; Helmholtz is reported, not gated. Both composition drivers write the CSV *first* and gate afterwards, so a long run keeps the measurements of every case that already succeeded -- which means **the presence of `adaptive-split-composition.csv` or `adaptive-split-composition-3d.csv` is not evidence of a successful run**: check the exit status, and check the rows' own `windowed_status` and mismatch columns. (The 2D driver used to validate before writing and leave no CSV on a gate failure; it now matches its 3D twin.) Either way, redirect stdout to a log: every row's mismatch is printed as it is produced. On a host exposing more than one OpenCL platform, set `PYOPENCL_CTX` to the intended platform, because the library's queue-less table-grid fallback would otherwise ask interactively and stall the run.
...
python benchmarks/adaptive_split_composition_3d.py --mode smoke

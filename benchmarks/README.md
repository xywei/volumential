...
The benchmark sweeps scalar Helmholtz wave numbers and Yukawa screening parameters in 2D by default; pass `--dim 3` for the corresponding 3D split/RKE path. Rows compare each split order against a fixed-parameter direct near-field table for that kernel parameter, giving an independent reference while holding the pretabulated split/RKE channel-table family fixed. Each row records split cache accounting. Full runs may use `--q-orders` and `--nlevels-list` to cover several local resolutions in one metadata-wrapped invocation, and `--split-smooth-quad-orders` to distinguish higher split-order effects from the smooth-remainder quadrature order. Pass `--helmholtz-k none` or `--yukawa-lambda none` for focused one-kernel probes. Full-mode 2D Yukawa direct-reference tables and Yukawa split channel tables use higher scalar-Duffy build orders than smoke mode, because the cheap default can under-integrate logarithmic channels and create a false split-error floor; the direct reference uses the stricter setting. Helmholtz rows keep the default split-channel build setting, which is already below the observed reference floor for the resolved cases. The CSV reports `effective_parameter=mu h`, a resolution metric (`points_per_wavelength` for Helmholtz and `nodes_per_decay_length` for Yukawa), and a coarse `resolved`/`transition`/`stress` label so paper tables can separate happy-path resolved cases from high-frequency or fast-decay stress cases. In `_row_from_result`, the `online_remainder_s` column is a conservative upper bound: it equals `split_warm_s` when `uses_online_remainder` is true, and is `0.0` otherwise, because the current wrangler instrumentation does not isolate smooth online-remainder work from the rest of the warm FMM/List-1 evaluation.

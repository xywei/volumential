...
but the split is the same: difficult local integrals are tabulated; well-separated
interactions stay on the ordinary FMM path.
```

## Why the orange boxes are special
...
The orange neighborhood contains many interactions that are equivalent under
